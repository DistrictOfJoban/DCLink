...
import com.lx862.dclink.commands.dclink;
...
public class DCLink implements ModInitializer {
	...
	public static void sendInGameMessage(List<MutableText> textToBeSent, MinecraftEntry entry) {
		...
		if(dimensions.isEmpty()) {
			//Send to all
			broadcastMessage(server, textToBeSent);
		} else {
	...
	public static void broadcastMessage(MinecraftServer server, List<MutableText> textToBeSent) {
